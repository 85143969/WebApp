 ...
 function insertBook(){
    db.transaction(function(tr){
     	...
     	tr.executeSql(insertSQL, [type, name], function(tr, rs){
      	    console.log('3_ 글 등록...no: ' + rs.insertId);
	        alert('게시글 ' + $('#bookName1').val() + ' 이 입력되었습니다');
	   		$('#bookName1').val('');
  ...
  db.transaction(function(tr){
 	var selectSQL = 'select * from book';
  	tr.executeSql(selectSQL, [], function(tr, rs){
       console.log(' 글 조회... ' + rs.rows.length + '건.');
       if (position == 'first') {
       	  if(index == 0)
       	  	alert('더 이상의 내용이 없습니다');
          else
          	index = 0;
	   	 }
    ...
    db.transaction(function(tr){
		...
		var updateSQL = 'update book set type = ?, name = ? where name = ?';
     	tr.executeSql(updateSQL, [type, new_name, old_name], function(tr, rs){
	         console.log('5_글 수정.... ') ;
	         alert('내용 ' + $('#sBookName2').val() + ' 이 수정되었습니다');
...
function deleteBook(){
   db.transaction(function(tr){
	  ...
	  tr.executeSql(deleteSQL, [name], function(tr, rs){
	     console.log('6_글 삭제... ');
	     alert('게시글 ' + $('#bookName3').val() + ' 이 삭제되었습니다');
